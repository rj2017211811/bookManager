...
public class InformationPal extends JPanel {
	 ...
	 * @throws Exception
	 */
	public InformationPal(List<String> itemNames, List<JComponent> items,
			List<JButton> btnCommond, int countOneLine, int itemNamesVis,
			int itemsVis) throws Exception {
		
		tableColNames = new Vector<String>(itemNames);
		lblItemNames = new ArrayList<JLabel>();
		for (int i = 0; i < itemNames.size(); i++)
		{
			JLabel jl=new JLabel(itemNames.get(i));
			team.javaGroup.bookManageSystem.gui.style.ComponentStyle.setLabelStyle(jl);
			lblItemNames.add(jl);
		}
		...
		for (int i = 0; i < countOneLine; i++) {
			...
			groupComponent[i] = layout
					.createParallelGroup(GroupLayout.Alignment.LEADING);
			for (int j = i; j < countOneLine
					* (lblItemNames.size() / countOneLine); j += countOneLine) {
		...
		GroupLayout.ParallelGroup groupline[] = new GroupLayout.ParallelGroup[lblItemNames
				.size() / countOneLine];
		...
		for (int k = countOneLine * (lblItemNames.size() / countOneLine); k < lblItemNames
				.size(); k++) {
		...
		//tblInfo.setBackground(new Color(0,255,255));
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				for (int i = 0; i < cptItems.size(); i++) {
					...
					if (c instanceof JTextComponent)
						...
						JList<String> list = (JList<String>) (((JScrollPane) c)
								.getViewport().getView());
	...
	public static void main(String[] args) {
		...
		btnCommond.add(new JButton("按钮1"));
		btnCommond.add(new JButton("按钮2"));
		btnCommond.add(new JButton("按钮3"));
		btnCommond.add(new JButton("按钮4"));
		try {
			InformationPal pal = new InformationPal(lblItemNames, cptItems,
					btnCommond, 3, 1, 1);
